...
const Auth = () => {
  ...
  // If already logged in, restore pending data and redirect
  useEffect(() => {
    if (user) {
  ...
  const handlePostAuthRedirect = async () => {
    ...
    const pendingCheckout = localStorage.getItem("scorecv_pending_checkout");
    const savedData = localStorage.getItem("scorecv_data");

    if (pendingCheckout === "true" && savedData) {
      // Save analysis to DB first
      try {
        const s = JSON.parse(savedData);
        if (s.cvText && s.targetJob && s.results && user) {
          await supabase.from("user_analyses").insert({
            user_id: user.id,
            cv_text: s.cvText,
            target_job: s.targetJob,
            job_description: s.jobDescription || "",
            industry: s.industry || "",
            results: s.results as any,
            score: s.results.score || 0,
            match_score: s.results.matchScore || null,
            is_paid: false,
          });
        }
      } catch { /* ignore */ }

      localStorage.removeItem("scorecv_pending_checkout");
  ...
  const handleSubmit = async (e: React.FormEvent, isLogin: boolean) => {
    ...
    try {
      ...
      // handlePostAuthRedirect will be triggered by the useEffect on user change
    } catch (err: any) {
  ...
  const EmailForm = ({ isLogin }: { isLogin: boolean }) => (
    <form onSubmit={(e) => handleSubmit(e, isLogin)} className="space-y-4">
      <div>
        <label className="label-ui block mb-1.5">Email</label>
        <div className="relative">
          ...
          <input
            ...
            required
            className="w-full pl-10 pr-4 py-3 bg-secondary rounded-xl text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-primary focus:outline-none"
          />
        </div>
      </div>
      <div>
        <label className="label-ui block mb-1.5">Mot de passe</label>
        <div className="relative">
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full py-4 bg-primary text-primary-foreground rounded-xl font-bold text-lg hover:opacity-90 transition-all disabled:opacity-50"
      >
  ...
  const GoogleButton = () => (
    <button
      ...
      className="w-full flex items-center justify-center gap-3 py-3.5 px-4 bg-background border border-border rounded-xl font-medium text-foreground hover:bg-secondary/50 transition-all"
    >
